...
class Vlr:
    ...
    def vlr_stats(region: str, timespan: int):
        ...
        for container in containers:
            ...
            adr = stats_container[4].div.text.strip()
            kpr = stats_container[5].div.text.strip()
            apr = stats_container[6].div.text.strip()
            fkpr = stats_container[7].div.text.strip()
            fdpr = stats_container[8].div.text.strip()
            hs = stats_container[9].div.text.strip()
            cl = stats_container[10].div.text.strip()

            result.append(
                {
                    "player": player,
                    "org": org,
                    "average_combat_score": kd,
                    "kill_deaths": acs,
